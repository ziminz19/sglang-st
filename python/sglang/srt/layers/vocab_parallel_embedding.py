...
class VocabParallelEmbedding(torch.nn.Module):
    ...
    def weighted_forward(
        self, topk_probs: torch.Tensor, topk_indices: torch.Tensor
    ) -> torch.Tensor:
        ...
        assert topk_probs.shape == topk_indices.shape, (
            "topk_probs and topk_indices must have same shape."
        )

        # Use quant_method.embedding for consistency
        topk_embeddings = self.quant_method.embedding(
            self, topk_indices.long()
        )  # [B, K, D]
    ...
    def weighted_forward_tp(
        ...
        topk_embeddings: torch.Tensor = self.quant_method.embedding(
            self, masked_indices.long()
        )  # [B, K, D]
    ...
    def get_masked_indices_and_mask(
        ...
        """
        vocab_mask = (indices >= org_vocab_start_index) & (
